...
class TWIG_KGL_v0(nn.Module):
    ...
    def __init__(self, n_local):
        ...
        The arguments it accepts are:
            - n_local (int)the number of graph structural features
              that are present in the input features vectors.
    ...
    def forward(self, X):
        ...
        architecture. For TWIG-I v0, this achitecture is approximately as
        follows.
            - all features are passed through three dense layers
...
class TWIG_KGL_v1(nn.Module):
    ...
    def forward(self, X):
        '''
        forward() defines the forward pass of the NN and the neural
        architecture. For TWIG-I v1, this is done as a basic linear
        regression task, without multiple hidden layers
        '''
        X = self.linear_final(X)
        X = self.sigmoid_final(X) #maybe use softmax instead,but I like this for now theoretically
